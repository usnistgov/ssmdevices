    ...
    def open(self):
        ...
        if len(specs) > 1:
            if self.resource is None:
                raise Exception(
                    'More than one connected USB device matches model ' + str(self.MODEL) + ' - provide serial number?'
                )
            else:
                raise Exception(
                    'More than one connected USB device match model ' + str(self.MODEL) + ' and serial ' + str(
                        self.resource
                    )
                )
        elif len(specs) == 0:
            raise Exception(
                'No USB devices connected that match model ' + str(self.MODEL) + ' and serial ' + str(
                    self.resource
                )
            )
