...
@attr.adjust('binary_path', 'C:\\Windows\\System32\\netsh.exe')
...
class WLANInfo(lb.ShellBackend):
    ...
    def get_wlan_ssids(self, interface):
        ...
        lines = re.findall('^\\s*(.*?)\\s*:\\s*(.*?)\\s*$', txt, flags=re.MULTILINE)
    ...
    def get_wlan_interfaces(self, name=None, param=None):
        ...
        lines = re.findall('^\\s*(\\S+.*?)\\s+:\\s+(\\S+.*?)\\s*$', txt, flags=re.MULTILINE)
        return pairs_to_dict(lines)


class WLANClient(lb.Device):

    def __init__(self, resource: str='Undefined', ssid: str='Undefined', timeout: str='int'):
        ...
    ...
    def open(self):
        ...
        if (
            self.resource not in available_by_interface.keys() and
            self.resource not in available_by_interface.values()
        ):
            txt = f'resource \'{self.resource}\' does not match any of the available WLAN interface names {tuple(
                available_by_interface.keys()
            )} or corresponding MAC addresses {tuple(available_by_interface.values())}'
            raise ConnectionError(txt)
        info = network_interface_info(self.resource)
        mac = info['physical_address']
        if mac not in available:
            txt = f'interface with MAC address \'{self.resource}\' is not one of the WLAN interfaces {tuple(
                available
            )}'
        ...
        self._logger.debug(
            f'client network interface is \'{info["interface"]}\' at physical address \'{info["physical_address"]}\''
        )
    ...
    def interface_disconnect(self):
        ...
        while time.perf_counter() - t0 < self.timeout:
            ...
            raise TimeoutError('tried to disconnect but only achieved the {} state '.format(repr(s)))
    ...
    def refresh(self):
        for attr in self._property_attrs:
            getattr(self, attr)
if __name__ == '__main__':
    client = WLANClient(resource='f8:ac:65:c8:72:bf', ssid='EnGenius1', timeout=5)
    with client:
        pass
