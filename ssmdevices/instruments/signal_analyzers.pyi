...
class KeysightN9951B(lb.VISADevice):
    ...
    frequency_span = attr.property.float(key='FREQ:SPAN', min=2, max=44000000000.0, label='Hz')
    ...
    reference_level = attr.property.float(key='DISP:WIND:TRAC1:Y:RLEV', step=0.001, label='dB')
    resolution_bandwidth = attr.property.float(key='BAND', min=1000.0, max=5760000.0, label='Hz')
...
class RohdeSchwarzFSWBase(lb.VISADevice):
    ...
    default_window: str = attr.value.str(
        default='',
        cache=True,
        help='data window number to use if unspecified'
    )
    default_trace: str = attr.value.str(
        default='',
        cache=True,
        help='data trace number to use if unspecified'
    )
    frequency_center = attr.property.float(key='FREQ:CENT', min=0, step=1e-09, label='Hz')
    frequency_span = attr.property.float(key='FREQ:SPAN', min=0, step=1e-09, label='Hz')
    frequency_start = attr.property.float(key='FREQ:START', min=0, step=1e-09, label='Hz')
    frequency_stop = attr.property.float(key='FREQ:STOP', min=0, step=1e-09, label='Hz')
    ...
    initiate_continuous = attr.property.bool(key='INIT:CONT')
    reference_level = attr.property.float(key='DISP:TRAC1:Y:RLEV', step=0.001, label='dB')
    reference_level_trace2 = attr.property.float(key='DISP:TRAC2:Y:RLEV', step=0.001, label='dB')
    reference_level_trace3 = attr.property.float(key='DISP:TRAC3:Y:RLEV', step=0.001, label='dB')
    reference_level_trace4 = attr.property.float(key='DISP:TRAC4:Y:RLEV', step=0.001, label='dB')
    reference_level_trace5 = attr.property.float(key='DISP:TRAC5:Y:RLEV', step=0.001, label='dB')
    reference_level_trace6 = attr.property.float(key='DISP:TRAC6:Y:RLEV', step=0.001, label='dB')
    amplitude_offset = attr.property.float(key='DISP:TRAC1:Y:RLEV:OFFS', step=0.001, label='dB')
    ...
    def acquire_spectrogram(self, acquisition_time_sec):
        ...
        while active_time == 0:
            ...
            lb.sleep(0.1)
            t0_active = time.time()
            with self.overlap_and_block(timeout=int(1000.0 * time_remaining)):
    ...
    def apply_channel_type(self, type_=None):
        ...
        if DEFAULT_CHANNEL_NAME in channel_list:
            self.write(
                f'INST:CRE:REPL \'{DEFAULT_CHANNEL_NAME}\',{self.channel_type},\'{DEFAULT_CHANNEL_NAME}\''
            )
        else:
    ...
    def fetch_timestamps(self, window=None, all=True, timeout=50000):
        ...
        try:
            ...
            ret = timestamps[:, 0] + 1e-09 * timestamps[:, 1]
        finally:
            if all:
    ...
    def fetch_spectrogram(self, window=None, freqs='exact', timestamps='exact', timeout=None):
        ...
        if timeout is None:
            ...
            old_timeout, self.backend.timeout = self.backend.timeout, 6 * 1000.0 * max_trigger_time
        else:
            old_timeout = timeout
    ...
    def get_marker_enables(self):
        ...
        states = [
            [self.query(f'CALC:MARK{m}:STATE?'), self.query(f'CALC:MARK{m}:FUNC:BPOW:STATE?')]
            for m in markers
        ]
...
class _RSLTEAnalyzerMixIn(RohdeSchwarzFSWBase):
    ...
    def uplink_sample_rate(self):
        ...
        return float(response[2:].replace('_', '.')) * 1000000.0
    ...
    def fetch_power_vs_symbol_x_carrier(self, window, trace):
        ...
        Ncarrier = int(50 * self.uplink_sample_rate / 10000000.0)
...
class _RSIQAnalyzerMixIn(RohdeSchwarzFSWBase):
    ...
    iq_sample_rate = attr.property.float(key='TRAC:IQ:SRAT', min=1e-09, max=160000000.0)
...
class _RSRealTimeMixIn(RohdeSchwarzFSWBase):
    ...
    sweep_dwell_time = attr.property.float(key='SWE:DTIM', min=0.03)
    ...
    def setup_spectrogram(
        ...
        if self.sweep_time_window2 != time_resolution:
            self._logger.warning(
                f'requested time resolution {time_resolution}, but instrument adjusted to {self.sweep_time_window2}'
            )
    ...
    def acquire_spectrogram_sequence(self, loop_time=None, delay_time=0.1, timestamps='fast'):
        ...
        while loop_time is None or time_remaining > 0:
            ...
            t0_active = time.time()
            with self.suppress_timeout():
                with self.overlap_and_block(timeout=int(1000.0 * 30 * max_trigger_time)):
                    self.trigger_single(wait=False)
                lb.sleep(0.05)
                active_time += time.time() - t0_active
                self.backend.timeout = 6 * 1000.0 * max_trigger_time
...
@attr.adjust('frequency_center', max=26500000000.0)
@attr.adjust('frequency_span', max=26500000000.0)
@attr.adjust('frequency_start', max=26500000000.0)
@attr.adjust('frequency_stop', max=26500000000.0)
@attr.adjust('resolution_bandwidth', min=45000.0, max=5760000.0)
...
@attr.adjust('frequency_center', max=43500000000.0)
@attr.adjust('frequency_span', max=43500000000.0)
@attr.adjust('frequency_start', max=43500000000.0)
@attr.adjust('frequency_stop', max=43500000000.0)
@attr.adjust('resolution_bandwidth', min=1, max=10000000.0)
